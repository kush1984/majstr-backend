...
- **Object-level payments (`project_payment`, V93)** — belongs to the **project**, not an estimate (an object usually has several estimates but one money arrangement); absorbed the old `Estimate.depositAmount` (data-migrated, column left in schema unread — drop deferred to open-questions). `amount` (planned) vs `paidAmount`/`paidAt` (actual) are separate fields; status is **derived, never stored** (`ProjectPayment.status(LocalDate today)` — PARTIAL beats OVERDUE beats PLANNED). `due_date`/`next_stage` are framed as a condition to unblock the next stage, never a debt reminder — same wording rule applies to any UI or copy touching them. `PaymentService` has **no `Feature` gate** — payments are FREE + PRO, only owner-scoped.
