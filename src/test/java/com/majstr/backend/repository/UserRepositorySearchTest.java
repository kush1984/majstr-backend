...
class UserRepositorySearchTest {
    ...
    void searchAdmin_withAUuid_passesItAsAnIdAsWellAsAPattern() {
        ...
        when(repo.searchAdmin(any(), any(), any(), any(), any(), any())).thenCallRealMethod();
        when(repo.searchAdminByPattern(any(), any(), any(), any(), any(), any(), any()))
                .thenReturn(Page.<com.majstr.backend.entity.User>empty());
        Instant activeSince = Instant.now();
        Instant recentSince = activeSince.minusSeconds(86_400);
        String raw = "866feca8-dc5b-403f-993e-c6a1b2c3d4e5";

        repo.searchAdmin(null, null, raw, activeSince, recentSince, Pageable.unpaged());
        ...
        verify(repo).searchAdminByPattern(
                ...
                eq(activeSince), eq(recentSince), eq(Pageable.unpaged()));
    }
    ...
    void searchAdmin_withText_delegatesWithLoweredPatternAndPlan() {
        UserRepository repo = mock(UserRepository.class);
        when(repo.searchAdmin(any(), any(), any(), any(), any(), any())).thenCallRealMethod();
        when(repo.searchAdminByPattern(any(), any(), any(), any(), any(), any(), any())).thenReturn(Page.<com.majstr.backend.entity.User>empty());
        Instant activeSince = Instant.now();
        Instant recentSince = activeSince.minusSeconds(86_400);

        repo.searchAdmin(Plan.PRO, null, "ACME", activeSince, recentSince, Pageable.unpaged());

        // text + plan together: lowered "%acme%" pattern, plan passed through, no source filter.
        verify(repo).searchAdminByPattern(eq(Plan.PRO), isNull(), eq("%acme%"), isNull(),
                eq(activeSince), eq(recentSince), eq(Pageable.unpaged()));
    }
    ...
    void searchAdmin_blankText_passesNullPattern_soAllUsersReturn() {
        ...
        repo.searchAdmin(null, null, "   ", activeSince, recentSince, Pageable.unpaged());

        // empty search → null pattern (no filter); no plan/source filter either.
        verify(repo).searchAdminByPattern(isNull(), isNull(), isNull(), isNull(),
                eq(activeSince), eq(recentSince), eq(Pageable.unpaged()));
    }
    ...
    void searchAdmin_trimsAndUppercasesTheSourceFilter() {
        ...
        repo.searchAdmin(null, " liga ", null, activeSince, recentSince, Pageable.unpaged());

        // source is trimmed + uppercased to match the stored referral_source.
        verify(repo).searchAdminByPattern(isNull(), eq("LIGA"), isNull(), isNull(),
                eq(activeSince), eq(recentSince), eq(Pageable.unpaged()));
    }
    ...
    void searchAdmin_registrationDescending_delegatesToTheOriginalTwoLevelQuery() {
        UserRepository repo = mock(UserRepository.class);
        when(repo.searchAdmin(any(), any(), any(), any(), any(), anyBoolean(), any())).thenCallRealMethod();
        when(repo.searchAdmin(any(), any(), any(), any(), any(), any())).thenCallRealMethod();
        when(repo.searchAdminByPattern(any(), any(), any(), any(), any(), any(), any())).thenReturn(Page.<com.majstr.backend.entity.User>empty());
        Instant activeSince = Instant.now();
        Instant recentSince = activeSince.minusSeconds(86_400);

        repo.searchAdmin(Plan.PRO, null, "ACME", activeSince, recentSince, false, Pageable.unpaged());

        // false (the default / DESC toggle state) is the same active-first, newest-registered-first
        // query as the 6-arg searchAdmin — no separate ORDER BY variant needed for it.
        verify(repo).searchAdminByPattern(eq(Plan.PRO), isNull(), eq("%acme%"), isNull(),
                eq(activeSince), eq(recentSince), eq(Pageable.unpaged()));
    }
    ...
    void searchAdmin_registrationAscending_delegatesToTheAscendingQuery() {
        UserRepository repo = mock(UserRepository.class);
        when(repo.searchAdmin(any(), any(), any(), any(), any(), anyBoolean(), any())).thenCallRealMethod();
        when(repo.searchAdminByPatternRegistrationAscending(any(), any(), any(), any(), any(), any(), any()))
                .thenReturn(Page.<com.majstr.backend.entity.User>empty());
        Instant activeSince = Instant.now();
        Instant recentSince = activeSince.minusSeconds(86_400);

        repo.searchAdmin(Plan.PRO, " liga ", "ACME", activeSince, recentSince, true, Pageable.unpaged());
        ...
        verify(repo).searchAdminByPatternRegistrationAscending(
                eq(Plan.PRO), eq("LIGA"), eq("%acme%"), isNull(),
                eq(activeSince), eq(recentSince), eq(Pageable.unpaged()));
    }
}
