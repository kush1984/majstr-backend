...
-- Every default template position must resolve to exactly one catalog row of its trade.
DO $$
DECLARE bad int;
BEGIN
  ...
  IF bad > 0 THEN
    RAISE EXCEPTION 'дедуп лишив % позицій шаблонів без позиції в каталозі', bad;
  END IF;
END $$;
