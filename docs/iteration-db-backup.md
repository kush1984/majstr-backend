...
- **`.github/workflows/db-backup.yml`** — scheduled (`cron: "0 3 * * *"`, 03:00
  ...
  1. Install the **PostgreSQL 18 client** from the PGDG apt repo (prod is
     PostgreSQL 18 on Railway; `pg_dump` must be ≥ server). **Gotcha (two
     traps):** (a) `postgresql-client` is a *virtual* package, so
     `postgresql-client=18.*` errors `Version '18.*' ... not found` — install
     the real versioned package `postgresql-client-18`. (b) The runner
     preinstalls the v16 client which owns `/usr/bin/pg_dump`; installing v18
     beside it doesn't change which binary wins, so `pg_dump` stays 16 ("server
     version mismatch"). Fix: after installing `postgresql-client-18`, prepend
     `/usr/lib/postgresql/18/bin` to `$GITHUB_PATH` so the v18 `pg_dump` wins in
     the dump step; verify in the install step via the explicit v18 path.
     Runner pinned to `ubuntu-22.04` (jammy) so the PGDG codename matches the OS
     and isn't subject to ubuntu-latest drift.
